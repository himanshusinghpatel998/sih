...
| `WASTE_MANAGEMENT/NagarAI-main/`   | Earlier MERN version + ESP32 firmware                          | Historical baseline `nagarai/` was forked from — not touched |
...
6. **Motion baseline**: add Framer Motion; use `animate`/`apple-design` skills for page transitions, sidebar/topbar micro-interactions, and the map's marker state changes (bin turning red as risk rises should _animate_, not snap).
