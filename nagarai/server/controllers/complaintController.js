...
const getComplaintById = async (req, res) => {
  try {
    ...
    if (req.user.role === "collector" && complaint.block !== req.user.block) {
      return res
        .status(403)
        .json({ message: "Not authorized to view other blocks" });
    }
...
const submitComplaint = async (req, res) => {
  try {
    ...
    let imageUrl = null;
    console.log(
      ` [SUBMIT] req.file present: ${!!req.file}`,
      req.file
        ? {
    ...
    if (req.file) {
      try {
        imageUrl = await uploadToCloudinary(req.file, "NagarAI/complaints");
...
const updateComplaintStatus = async (req, res) => {
  try {
    ...
    if (req.user.role === "collector" && complaint.block !== req.user.block) {
      return res
        .status(403)
        .json({ message: "Not authorized to update other blocks" });
    }
...
const completeComplaint = async (req, res) => {
  try {
    ...
    if (assignedId && assignedId !== userId) {
      ...
      return res
        .status(403)
        .json({ message: "Only the assigned collector can complete this." });
    }
    ...
    try {
      ...
      imageUrl = await uploadToCloudinary(req.file, "NagarAI/completions");
